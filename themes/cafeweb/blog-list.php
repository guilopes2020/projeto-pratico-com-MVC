<article class="blog_article">
    <a title="<?= $post->title; ?>" href="<?= url("/blog/{$post->uri}"); ?>">
        <img title="Blog" alt="<?= $post->title; ?>" src="<?= image($post->cover, 600, 340); ?>"/>
    </a>
    <header>
        <p class="meta"><?= $post->category()->title ?> &bull; Por <?= "{$post->author()->first_name} {$post->author()->last_name}"; ?> &bull; <?= date_fmt($post->post_at); ?></p>
